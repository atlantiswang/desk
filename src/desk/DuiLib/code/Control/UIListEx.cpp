...
namespace DuiLib {
	...
	{
		CListContainerHeaderItemUI* pHItem = static_cast<CListContainerHeaderItemUI*>(m_pHeader->GetItemAt(nColum)->GetInterface(_T("ListContainerHeaderItem")));
		if (pHItem == NULL)
		{
			return;
		}
		...
		if (bChecked)
		{
			...
			for(int i = 0; i < GetCount(); i++) 
			{
				CListTextExtElementUI* pLItem = static_cast<CListTextExtElementUI*>(GetItemAt(i)->GetInterface(_T("ListTextExElement")));
	...
	void CListContainerHeaderItemUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        else if (_tcsicmp(pstrName, _T("ellipsis")) == 0) {
            if (_tcsicmp(pstrValue, _T("end")) == 0) m_uTextStyle |= DT_END_ELLIPSIS;
            else if (_tcsicmp(pstrValue, _T("mid")) == 0) m_uTextStyle |= DT_PATH_ELLIPSIS;
            else m_uTextStyle &= ~(DT_END_ELLIPSIS | DT_PATH_ELLIPSIS);
        }
