...
namespace DuiLib {
	...
	void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        else if (_tcsicmp(pstrName, _T("itemellipsis")) == 0) {
            if (_tcsicmp(pstrValue, _T("end")) == 0) m_ListInfo.uTextStyle |= DT_END_ELLIPSIS;
            else if (_tcsicmp(pstrValue, _T("mid")) == 0) m_ListInfo.uTextStyle |= DT_PATH_ELLIPSIS;
            else m_ListInfo.uTextStyle &= ~(DT_END_ELLIPSIS | DT_PATH_ELLIPSIS);
        }
